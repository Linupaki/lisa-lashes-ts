    ...
    list.innerHTML = cards.map(c => `
      <div class="addr-card ${c.is_default ? 'is-default' : ''}">
        ...
        <div class="addr-actions">
          <button class="btn-ghost danger" onclick="removeCard(${c.id})">Remove</button>
        </div>
      </div>`).join('');
  } catch (e) { }
}
...
async function changePassword() {
  ...
  if (next !== confirm) return showMsg('pw-msg', 'New passwords do not match.', false);
...
async function loadAddresses() {
  ...
  try {
    ...
    if (!res.ok) { list.innerHTML = '<div class="empty">Could not load addresses.</div>'; return; }
    ...
    list.innerHTML = addrs.map(a => `
      <div class="addr-card ${a.is_default ? 'is-default' : ''}">
        ...
        <div class="addr-actions">
          ${a.is_default ? '' : `<button class="btn-ghost" onclick="setDefaultAddress(${a.id})">Set default</button>`}
          <button class="btn-ghost" onclick='editAddress(${JSON.stringify(a)})'>Edit</button>
          <button class="btn-ghost danger" onclick="removeAddress(${a.id})">Delete</button>
        </div>
      </div>`).join('');
  } catch (e) {
...
function initDeleteAccount() {
  ...
  // Verify all elements are present in the DOM before assigning listeners
  if (!showModalBtn || !deleteModal || !cancelBtn || !confirmBtn) return;
  ...
  // 3. Close modal if clicking overlay background
  ...
  confirmBtn.addEventListener('click', async () => {
    ...
    try {
      const response = await fetch(API + '/account', {
        method: 'DELETE',
        credentials: 'include', // Uses the same Cookie configuration as your other profile requests
        headers: {
      ...
      // Redirect back to login/home screen after deletion
      window.location.href = 'account.html';
